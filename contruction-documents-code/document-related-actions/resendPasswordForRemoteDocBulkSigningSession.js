...
export async function resendPasswordForRemoteDocBulkSigningSession({
  slug,
  phone,
}) {
  ...
  try {
    ...
    const document = await prisma.workerBulkSigningSession.findFirst({
      where: { slug },
      include: {
        foreignWorker: {
          select: {
    ...
    console.log("Found document:", {
      ...
      hasWorker: !!document?.foreignWorker,
      phoneNumber: document?.foreignWorker?.israelPhoneNumber,
    });
    ...
    const updatedDocument = await prisma.workerBulkSigningSession.update({
    ...
    console.log("Sending SMS to:", document.foreignWorker.israelPhoneNumber);
    const isSuccess = await sendSms({
      phone: phone ?? document.foreignWorker.israelPhoneNumber,
