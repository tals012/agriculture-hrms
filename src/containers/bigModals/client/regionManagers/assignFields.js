...
import assignField from "@/app/(backend)/actions/regionManagers/assignField";
...
const AssignFields = ({ clientId }) => {
  ...
  const fetchManagers = async () => {
    try {
      const res = await getRegionManagers({ clientId });
      if (res?.status === 200) {
        setRegionManagers(res.data);
      } else {
  ...
  const handleAssign = async (fieldId) => {
    ...
    try {
      const res = await assignField({
        payload: {
          regionManagerId: selectedManager,
          fieldId,
        }
      });
